...
// --- Persisted state ---
let state = { blocks: {}, drInitials: {}, euthChecklist: {}, patientFields: {}, idexx: {}, editHistory: [] };
...
wss.on('connection', (ws) => {
    ...
    ws.send(JSON.stringify({ type: 'full-state', blocks: state.blocks, drInitials: state.drInitials, euthChecklist: state.euthChecklist, patientFields: state.patientFields, idexx: state.idexx, editHistory: state.editHistory }));
    ...
    ws.on('message', (raw) => {
        ...
        if (msg.type === 'update-block') {
            // Validate id is one of the known numeric block IDs (1–16)
            ...
            Object.assign(state.idexx, msg.data);
            saveState();
        } else if (msg.type === 'history-entry') {
            if (msg.entry && msg.entry.action) {
                state.editHistory.unshift(msg.entry);
                if (state.editHistory.length > 30) state.editHistory = state.editHistory.slice(0, 30);
                saveState();
            }
        }
